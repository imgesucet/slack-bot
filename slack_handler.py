...
class SlackRequestHandler:
    ...
    def handle(self, req: Request) -> Response:
        print(f"SlackRequestHandler, handle, method={req.method}, path={req.path}")
        if req.method == "GET":
            if req.path == "/healthcheck":
                return make_response("ok", 200)

            if self.app.oauth_flow is not None:
                print(f"SlackRequestHandler, handle, method={req.method}, path={req.path}, oauth_flow")
                oauth_flow: OAuthFlow = self.app.oauth_flow
                if req.path == oauth_flow.install_path:
                    print(f"SlackRequestHandler, handle, method={req.method}, path={req.path}, oauth_flow, install_path")
                    ...
                    print(f"SlackRequestHandler, handle, method={req.method}, path={req.path}, oauth_flow, redirect_uri_path")
        ...
        print(f"SlackRequestHandler, handle, method={req.method}, path={req.path}, oauth_flow, 404")
